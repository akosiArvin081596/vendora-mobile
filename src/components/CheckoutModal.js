          ...
          <View className="flex-[0.9] bg-vendora-card rounded-t-3xl">
            ...
            >
              ...
              <View className="mt-5">
                ...
                {/* First row: None, SC, PWD, Employee */}
                <View className="flex-row gap-2 mb-2">
                  {DISCOUNT_PRESETS.slice(0, 4).map((preset) => (
                    <TouchableOpacity
                      key={preset.id}
                ...
                {/* Second row: Promo, Custom */}
                <View className="flex-row gap-2">
                  {DISCOUNT_PRESETS.slice(4).map((preset) => (
                    <TouchableOpacity
                      key={preset.id}
                      className={`flex-1 py-2.5 rounded-xl ${
                        selectedDiscount === preset.id
                ...
                {selectedDiscount === 'promo' && (
                  <View className="mt-3">
                    ...
                    <Text className="text-vendora-text-muted text-xs mt-2">
                      Promo code validation coming soon
                    </Text>
                  </View>
                )}
                      ...
                      <TouchableOpacity
                        className={`flex-1 py-2 rounded-xl ${
                          customDiscountType === 'amount'
                            ? 'bg-vendora-purple/30 border border-vendora-purple'
                            : 'bg-vendora-input'
                        }`}
                ...
                {/* VAT Exempt Notice */}
                {currentPreset.vatExempt && (
                  <View className="mt-3 flex-row items-center bg-green-500/10 rounded-xl px-4 py-3">
                    <Ionicons name="information-circle" size={20} color="#22c55e" />
                    <Text className="text-green-400 text-sm ml-2 flex-1">
                      VAT Exempt applied - Tax automatically set to 0%
                    </Text>
                  </View>
                )}
                ...
                {discount > 0 && (
                  <View className="mt-3 flex-row items-center justify-between bg-vendora-bg rounded-xl px-4 py-3">
                    <View className="flex-row items-center">
                      <Ionicons name="pricetag" size={16} color="#22c55e" />
                      <Text className="text-vendora-text ml-2">
                        {currentPreset.label}
                      </Text>
                    </View>
